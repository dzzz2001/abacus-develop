...
void Grid_Technique::init_atoms_on_grid(const int& ny,
    ...
    for (int iat = 0; iat < ucell.nat; iat++) {
        ...
        for (int im = 0; im < this->meshball_ncells; im++) {
            ...
            const int normal
                = index2normal[this->index_atom[iat] + this->index_ball[im]];
#ifdef __DEBUG
            if (normal >= nbxyz) {
#pragma omp critical
                {
            ...
            const double dr_x_part = this->meshball_positions[im][0]
                                     - this->tau_in_bigcell[iat][0];
            const double dr_y_part = this->meshball_positions[im][1]
                                     - this->tau_in_bigcell[iat][1];
            const double dr_z_part = this->meshball_positions[im][2]
                                     - this->tau_in_bigcell[iat][2];
    ...
    for (int iat = 0; iat < ucell.nat; iat++) {
        ...
        for (int im = 0; im < this->meshball_ncells; im++) {
            ...
            if (is_atom_on_bcell) {
                ...
                const int index = this->bcell_start[bcell_idx_on_proc]
                                  + this->how_many_atoms[bcell_idx_on_proc];
...
void Grid_Technique::init_gpu_gint_variables(const UnitCell& ucell,
    ...
    // 2];
    double* psi_u_now = (double*)malloc(ucell.ntype * ucell.nwmax * this->nr_max
                                        * 2 * sizeof(double));
    memset(psi_u_now,
           0,
           ucell.ntype * ucell.nwmax * this->nr_max * 2 * sizeof(double));
    ...
    checkCudaErrors(cudaMalloc((void**)&psi_u_g,
                               ucell.ntype * ucell.nwmax * this->nr_max
                                   * sizeof(double) * 2));
    ...
    checkCudaErrors(
        cudaMalloc((void**)&mcell_pos_g, bxyz * 3 * sizeof(double)));
