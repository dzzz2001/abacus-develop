...
#pragma omp for schedule(dynamic)
        for(const auto& biggrid: gint_info_->get_biggrids())
        {
            ...
            phi_op.phi_mul_vldr3(vofk_, dr3_, dphi_x.get_ptr_1D(), dphi_x_vldr3.get_ptr_1D());
            phi_op.phi_mul_vldr3(vofk_, dr3_, dphi_y.get_ptr_1D(), dphi_y_vldr3.get_ptr_1D());
            phi_op.phi_mul_vldr3(vofk_, dr3_, dphi_z.get_ptr_1D(), dphi_z_vldr3.get_ptr_1D());
            phi_op.phi_mul_phi_vldr3(phi.get_ptr_1D(), phi_vldr3.get_ptr_1D(), &hRGint_local);
            phi_op.phi_mul_phi_vldr3(dphi_x.get_ptr_1D(), dphi_x_vldr3.get_ptr_1D(), &hRGint_local);
            phi_op.phi_mul_phi_vldr3(dphi_y.get_ptr_1D(), dphi_y_vldr3.get_ptr_1D(), &hRGint_local);
            phi_op.phi_mul_phi_vldr3(dphi_z.get_ptr_1D(), dphi_z_vldr3.get_ptr_1D(), &hRGint_local);
        }
#pragma omp critical
        {
